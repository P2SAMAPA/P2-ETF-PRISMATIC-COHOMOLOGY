...
st.set_page_config(page_title="Prismatic Cohomology", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🌀 Prismatic Cohomology</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Bhatt-Scholze p-adic geometry | Unifies étale, crystalline, and de Rham cohomologies</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Prismatic")
...
st.sidebar.markdown(f"**p-adic base:** {config.P_ADIC_BASE} | **Prismatic dim:** {config.PRISMATIC_DIM}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'prismatic_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Prismatic rank: {etf['prismatic_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Prismatic Rank"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Prismatic Rank", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🌀 Top ETFs by Prismatic Cohomology Rank (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Prismatic cohomology** (Bhatt-Scholze) unifies étale, crystalline, and de Rham cohomologies.
        - This provides a universal framework for analyzing discrete price structures.
        - The embedding captures: de Rham moments, crystalline p-adic valuations, and étale topological features.
        - The score is the **prismatic cohomology rank** – higher = more universal structure.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the prismatic cohomology ranks.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Prismatic Cohomology | Bhatt-Scholze p-adic geometry for universal structure")
