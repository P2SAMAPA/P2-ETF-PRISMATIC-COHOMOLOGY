...
from prismatic_cohomology import prismatic_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    for ticker in ret_window.columns:
        ...
        s = prismatic_score(
            ...
            macro_window,
            p=config.P_ADIC_BASE,
            dim=config.PRISMATIC_DIM,
            unification_levels=config.UNIFICATION_LEVELS
        )
    ...
    top_etfs = [{"ticker": t, "prismatic_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    results = {
        ...
        "p_adic_base": config.P_ADIC_BASE,
        "prismatic_dim": config.PRISMATIC_DIM,
        "unification_levels": config.UNIFICATION_LEVELS,
    ...
    out_file = f"output/prismatic_{timestamp}.json"
